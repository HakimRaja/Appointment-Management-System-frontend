...
const DateInput = ({label,required,name,value,onChange,format = 'YYYY-MM-DD' ,labelClass , datePickerClass, disabledDateFunc = false}) => {
  return (
    <div className='my-4'>
        {label && (<label className={labelClass ? labelClass:('font-semibold mt-4')}>{label.slice(0,25)}{required && (<span className='text-red-800'>*</span>)} <br /> <span className='font-semibold '>{label.slice(25)}</span></label>)}
        <DatePicker
        name = {name}
